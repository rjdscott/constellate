...
def test_records_exactly_samples_and_discards_warmup() -> None:
    ...
    assert result.recorded == 40  # histogram holds exactly the measured window
    ...
    assert result.completion_hz == pytest.approx(500, rel=0.5)
...
def test_errors_counted_not_recorded() -> None:
    ...
    result = asyncio.run(run_open_loop(request, rate_hz=1000, concurrency=2, samples=16, warmup=4))
    assert result.errors == 8  # measured window only
    assert result.warmup_errors == 2
    assert result.recorded == 8  # 16 samples - 8 errors
